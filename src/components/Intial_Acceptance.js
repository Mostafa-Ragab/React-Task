...
const IntialAcceptance = (props)=>{ 
    const StudentList  = ({studentList})=>{
        if(Object.keys(studentList).length>0)
        {
            return studentList.filter(student => student.Status.includes('Intial Acceptance')).map((student)=>{
                return(
                    <tr key={Math.random()}>  
                        <td>{student.Name}</td>
...
export default connect(mapStateToProps)(IntialAcceptance);