...
const ConditionalAcceptence = (props)=>{ 
    const StudentList  = ({studentList})=>{
        if(Object.keys(studentList).length>0)
        {
            return studentList.filter(student => student.Status.includes('Conditional Acceptance')).map((student)=>{
                return(
                    <tr key={Math.random()}>  
                        <td>{student.Name}</td>
...
export default connect(mapStateToProps)(ConditionalAcceptence);